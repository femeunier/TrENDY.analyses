...
# Test 2 with ET
all.df <- readRDS("./outputs/Trendy.global.evapotrans.v11.RDS") %>%
...
ggplot() +
  geom_tile(data = Trendy.wide.sum,
            aes(x = tas.cat, y = pr.cat,
                fill = diff_ET.m*86400),na.rm = TRUE, alpha = 1) +
  labs(x = "",y = "") +
  facet_wrap(~ model, scales = "free") +
  ...
  geom_text(data =Whittaker_biomes %>% group_by(biome) %>%
            ...
            size = 3.5)  +
  labs(x = "MAT (°C)",y = "MAP (cm)", fill = "ET difference \r\n (hist - cCO2) \r\n [kg/m²/d]") +
  theme_bw()
...
ggplot() +
  geom_contour_filled(data = df.perfect,
                      aes(x = temp_c, y = precp_cm, z= diff_ET.m*86400),
                      alpha = 0.4,
                      bins = 9) +
  geom_polygon(data = Whittaker_biomes,
               aes(x = temp_c, y = precp_cm, group = biome),
...
ggplot() +
  geom_contour_filled(data = df.perfect.sum,
                      aes(x = temp_c, y = precp_cm, z= diff_ET.m.m*86400),
                      alpha = 0.4,
                      bins = 9) +
  geom_polygon(data = Whittaker_biomes,
               aes(x = temp_c, y = precp_cm, group = biome),
...
ggplot() +
  geom_contour_filled(data = df.perfect.sum,
                      aes(x = temp_c, y = precp_cm, z= diff_ET.m.sd*86400),
                      alpha = 0.4,
                      bins = 9) +
  geom_polygon(data = Whittaker_biomes,
               aes(x = temp_c, y = precp_cm, group = biome),
